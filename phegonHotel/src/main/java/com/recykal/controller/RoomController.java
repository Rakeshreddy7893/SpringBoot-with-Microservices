...
import com.recykal.service.interfaces.IBookingService;
import com.recykal.service.interfaces.IRoomService;
...
public class RoomController {
    ...
    //@RequestParam(required = false) used to indicate that Query Parameters are Optional
    ...
    public ResponseEntity<Response> getAvailableRoomsByDateAndType(
        ...
        if(checkInDate == null || roomType == null || roomType.isBlank() || checkOutDate== null){
            Response response = new Response();
            response.setStatusCode(400);
            response.setMessage("Please provide values for all fields (chekInDate,checkOutDate and roomType");
            return  ResponseEntity.status(response.getStatusCode()).body(response);
        }
    ...
    public ResponseEntity<Response> deleteRoom(@PathVariable("roomId") Long roomId){
        Response response = roomService.deleteRooms(roomId);
        return  ResponseEntity.status(response.getStatusCode()).body(response);
    }
